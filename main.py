...
class SongTabFinder(BoxLayout):
    ...
    def record_audio(self, duration=5, sample_rate=44100):
        ...
        if platform.system() == "Windows":
            ...
            try:
                # SDL2 audio (Kivy) - limited mic support, faking for now
                self.sound = SoundLoader.load('microphone')  # Not fully functional yet
                if not self.sound:
                    print("Mic not available, faking audio")
                    time.sleep(duration)
                    return np.zeros(int(duration * sample_rate), dtype='float32')
                self.recording = True
                self.sound.play()
                Clock.schedule_once(self.stop_recording, duration)
                time.sleep(duration)  # Wait for recording
                # Placeholder: SDL2 mic needs custom patch
                return np.zeros(int(duration * sample_rate), dtype='float32')
            except Exception as e:
                print(f"Android audio error: {e}")
                return None
